...
export async function POST(req: NextRequest) {
  try {
    const { planKey, userEmail } = await req.json();
    ...
    // ✅ Stripe est accessible, retourner l'URL de paiement
    const checkoutUrl = `${plan.paymentLink}?prefilled_email=${encodeURIComponent(userEmail)}`;
