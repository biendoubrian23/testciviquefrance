...
export default function TestExamen2Page() {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center"><Loader2 className="w-12 h-12 animate-spin text-orange-500" /><p className="ml-4">Chargement Examen #2...</p></div>;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50 p-4">
        <div className="max-w-4xl mx-auto">
          ...
          <h2 className="text-2xl font-bold mb-4">📝 Correction - Examen #2</h2>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto p-4">
        <div className="bg-orange-500 text-white p-4 mb-6 flex items-center justify-between">
          <h1 className="text-xl font-bold">📝 Examen Blanc #2</h1>
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 px-3 py-1 rounded ${timeRemaining < 300 ? 'bg-red-600' : 'bg-orange-600'}`}>
          ...
          <div className="w-72 flex-shrink-0">
            <div className="bg-white border p-4 sticky top-4">
              ...
              <div className="grid grid-cols-5 gap-2 mb-4">
                {questions.map((q, i) => (
                  <button key={q.id} onClick={() => goToQuestion(i)} className={`w-9 h-9 rounded-full text-sm font-medium ${i === currentQuestionIndex ? 'bg-orange-500 text-white' : userAnswers[q.id] !== undefined ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-gray-100'}`}>{i+1}</button>
                ))}
              </div>
          ...
          <div className="flex-1">
            <div className="bg-white border p-6">
              <div className="flex justify-between mb-4">
                ...
                <span className="bg-orange-100 text-orange-700 px-3 py-1 text-sm rounded-full">Examen #2</span>
              </div>
              <p className="text-xl font-medium mb-6">{currentQuestion.question}</p>
              <div className="space-y-3">
                {currentQuestion.options.map((o, i) => (
                  <button key={i} onClick={() => handleAnswerSelect(i)} className={`w-full flex items-center gap-4 p-4 border-2 text-left ${selectedAnswer === i ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300'}`}>
                    <span className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${selectedAnswer === i ? 'bg-orange-500 text-white' : 'bg-gray-100'}`}>{['A','B','C','D'][i]}</span>
                    <span className="flex-1">{o}</span>
                    {selectedAnswer === i && <CheckCircle className="w-6 h-6 text-orange-500" />}
                  </button>
                ))}
              </div>
              <div className="flex justify-between mt-8 pt-6 border-t">
                ...
                <button onClick={goToNextQuestion} disabled={currentQuestionIndex === questions.length - 1} className={`flex items-center gap-2 px-4 py-2 rounded ${currentQuestionIndex === questions.length - 1 ? 'bg-gray-200 text-gray-400' : 'bg-orange-500 text-white hover:bg-orange-600'}`}>Suivant<ChevronRight className="w-5 h-5" /></button>
              </div>
            </div>
          </div>
        </div>
